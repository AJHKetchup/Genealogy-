...
def test_docling_discovery_keeps_ocr_for_noisy_scanned_text_layer(tmp_path, monkeypatch) -> None:
    fitz = pytest.importorskip("fitz")
    Image = pytest.importorskip("PIL.Image")
    init_genealogy_wiki(tmp_path)

    source = tmp_path / "raw" / "sources" / "scan-with-ocr-overlay.pdf"
    image_path = tmp_path / "page-scan.png"
    Image.new("RGB", (360, 500), "white").save(image_path)

    doc = fitz.open()
    page = doc.new_page(width=360, height=500)
    page.insert_image(page.rect, filename=str(image_path))
    noisy_overlay = "\n".join(["A"] * 20 + ["Usable OCR overlay text for Docling baseline. " * 12])
    page.insert_textbox((36, 36, 324, 460), noisy_overlay, fontsize=11)
    doc.save(source)
    ...
    def fake_docling(input_path, **kwargs):
        calls.append({"input_path": str(input_path), **kwargs})
        return "Usable OCR overlay text for Docling baseline. " * 12
    ...
    assert summary["tasks"][0]["text_layer_flags"]
...
def test_docling_discovery_caps_pages_per_source(tmp_path, monkeypatch) -> None:
    fitz = pytest.importorskip("fitz")
    init_genealogy_wiki(tmp_path)
    source = tmp_path / "raw" / "sources" / "large-one-source.pdf"
    doc = fitz.open()
    for page_number in range(5):
        page = doc.new_page(width=360, height=500)
        page.insert_textbox((36, 36, 324, 460), f"Usable printed source text page {page_number}. " * 8, fontsize=11)
    doc.save(source)

    prepare_raw_sources(tmp_path, new_pages_limit=5)
    write_agent_queues(tmp_path)
    ...
    assert summary["inspected"] == 2
    assert summary["accepted"] == 2
    assert summary["skipped"]["max_pages_per_source_reached"] == 3
...
def test_docling_discovery_reports_tesseract_failure_after_docling_error(tmp_path, monkeypatch) -> None:
    fitz = pytest.importorskip("fitz")
    init_genealogy_wiki(tmp_path)
    source = tmp_path / "raw" / "sources" / "scanned-tesseract-timeout.pdf"
    doc = fitz.open()
    doc.new_page(width=360, height=500)
    doc.save(source)
