# Conversion QC Triage: ca07263f40-cv-of-dario-arturo-pulga-p0004-0009-cv-of-dario-arturo-pulgar-pages-4-9.codex

- Converted file: `raw/converted/ca07263f40-cv-of-dario-arturo-pulga-p0004-0009-cv-of-dario-arturo-pulgar-pages-4-9.codex.md`
- Chunk manifest: `raw/chunks/ca07263f40-cv-of-dario-arturo-pulga-p0004-0009-cv-of-dario-arturo-pulgar-pages-4-9-codex/manifest.json`
- Rule: do not edit converted Markdown here; queue exact pages or regions for reread.

## Page Summary

| Page | Relevance | Confidence | Action | Flags | Matched Terms |
| --- | --- | --- | --- | --- | --- |
| 4 | high | medium | reread-page | explicit_reread_needed | Arturo, Dario, Entry, Pulgar |
| 5 | high | high | pass | none | Arturo, Dario, Pulgar |
| 6 | high | medium | reread-page | explicit_reread_needed | Arturo, Dario, Entry, Pulgar |
| 4 | medium | high | pass | none | Entry |
| 8 | high | high | pass | none | Arturo, Dario, Entry, Pulgar |
| 6 | high | medium | reread-page | possible_table_layout_loss | Arturo, Dario, Entry, Pulgar |
