...
Reviewed the original page image, assigned chunk, current converted Markdown, and prior staged conflict notes for register page 58, entry 172. This note is staged under the current worker id and does not edit raw sources, converted Markdown, chunks, or page Markdown.
...
Use the image-reviewed row for staged evidence drafts, preserving the converted-Markdown conflict in every dependent claim or relationship candidate. Dependent drafts may proceed to proof review with `promotion_recommendation: promote_after_review`, but no canonical claim, relationship, identity merge, parent merge, or wiki update should occur until proof review accepts this QA-controlled reading.
