...
Reviewed the original page image, assigned chunk, current converted Markdown, and staged source-packet context for register page 58, entry 172. This note does not edit raw sources, converted Markdown, chunks, or page Markdown.

## Controlling Row Certification

The physical row controlling entry `172` is the middle row on register page 58. It is the Pulgar/Arriagada row, not the Burgos/de la Cruz row currently described in the converted Markdown.
...
The father field is certified only to the visible reading `Jose del Carmen Pulgar`. The assigned chunk's trailing `S.` after `Pulgar` is not clear enough in the image to certify.
...
Use the image-reviewed row for staged evidence drafts, preserving the converted-Markdown conflict in dependent claims and relationship candidates. These drafts may proceed to proof review with `promotion_recommendation: promote_after_review`, but no canonical claim, relationship, identity merge, parent merge, or wiki update should occur until proof review accepts this QA-controlled reading.
