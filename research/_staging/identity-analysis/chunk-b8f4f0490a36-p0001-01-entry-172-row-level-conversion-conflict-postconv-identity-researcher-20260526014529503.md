...
worker: "postconv-identity-analysis-20260526014529503"
...
1. The exact staged draft analyzed here is `research/_staging/conflicts/chunk-b8f4f0490a36-p0001-01-entry-172-row-level-conversion-conflict-postconv-evidence-extraction-20260526002943637.md`.
2. Row control is unresolved: the assigned chunk/source packet present entry 172 as a Pulgar/Arriagada row, while the current converted Markdown presents entry 172 as a Burgos/de la Cruz row.
3. This is not a spelling-only conflict. The disagreement changes child identity, parents, birth date/time/place, parent residence, and informant.
4. The father field is not promotion-ready. The Pulgar reading must be certified only to the visible extent: `Jose del Carmen Pulgar`, `Jose del Carmen Pulgar S.`, or `Jose del Carmen Pulgar [?]`.
5. Existing wiki pages already preserve related Pulgar/Jose/Juana/Dario clusters, but this draft does not authorize any merge, rename, relationship promotion, or canonical fact strengthening.
6. No external research was performed. Raw sources, converted Markdown, chunks, and canonical wiki pages were not edited.

## Literal Evidence

From the staged draft and source packet, the Pulgar/Arriagada reading for entry 172 is:

- Child: `Jose del Carmen Segundo Pulgar Arriagada`.
- Sex: `Hombre`.
- Birth: 8 March 1888, about 3 p.m., `Calle de Valdivia`.
- Father: `Jose del Carmen Pulgar S.` in the assigned chunk, with exact suffix needing QA.
- Mother: `Juana Arriagada de Pulgar`.
- Informant: `Ernesto Herrera L.`, present at birth.

From the current converted Markdown, the competing entry-172 reading is:
...
- `wiki/people/jose-del-carmen-segundo-pulgar-arriagada.md` has generated entry-172 life facts and a probable mother link to `Juana Arriagada de Pulgar`.
- `wiki/people/juana-arriagada-de-pulgar.md` has a probable child link to `Jose del Carmen Segundo Pulgar Arriagada`.
- `wiki/people/jose-del-carmen-pulgar.md` and `wiki/people/juana-de-dios-amagada-de-pulgar.md` preserve separate Jose/Juana parent-candidate context from another child cluster.
- `wiki/people/dario-arturo-pulgar-smith.md` is family-supplied and explicitly warns not to attach Dario/Pulgar/Pulgar-Arriagada/Pulgar-Smith records without identity review.
- `wiki/people/dar-o-pulgar-arriagada.md`, `wiki/people/dario-pulgar-adult-passenger-age-64.md`, and `wiki/people/dario-pulgar-child-passenger-age-11.md` preserve separate Dario/Pulgar clusters with no bridge supplied by this entry-172 draft.
...
- The staged source packet says this row is family-relevant if targeted conversion QA certifies the controlling row.
- The staged conflict draft states that the assigned chunk supports the Pulgar/Arriagada row.

Conflicts and limits:

- The current converted Markdown assigns the same entry number to the Burgos/de la Cruz row.
- The final father-name form after `Pulgar` is not certified.
- Even if confirmed, this row proves a Jose del Carmen Segundo entry, not a Dario identity.
...
| conflict_severity | 0.95 | Competing rows identify different children, parents, places, and dates. |
...
| relevance | 0.94 | Directly relevant to Pulgar/Arriagada line only if QA confirms it. |
...
- The staged draft already identifies the converted Markdown as the conflict side.
...
| conflict_severity | 0.95 | Acceptance would invalidate the staged Pulgar entry-172 family claims. |
...
- The Pulgar row names father `Jose del Carmen Pulgar` or `Jose del Carmen Pulgar S.` and mother `Juana Arriagada de Pulgar`.
- Existing wiki context has `Jose del Carmen Pulgar` as a separate stub.
- Existing wiki context has both `Juana Arriagada de Pulgar` and `Juana de Dios Amagada de Pulgar`, making the `Arriagada`/`Amagada` distinction a necessary anti-conflation check.

Conflicts and limits:

- This draft does not mention `Tulio Cesar Luis Jose`, the separate cluster linked to `Jose del Carmen Pulgar` and `Juana de Dios Amagada de Pulgar`.
- This draft does not prove `Juana Arriagada de Pulgar` and `Juana de Dios Amagada de Pulgar` are the same person.
- Row control and the father suffix must be resolved before any parent identity review can be decisive.
...
- Existing wiki/staged context includes multiple Dario/Pulgar clusters, so anti-conflation comparison is required.

Required Pulgar-line comparison:

- `Dario Arturo Pulgar-Smith`: this entry does not name Dario, Arturo, Smith, Pulgar-Smith, Alexander John Heinz, or a family relationship to the canonical family-supplied page.
- `Dario Arturo Pulgar`: this entry does not name Dario Arturo Pulgar or any CV/document-subject bridge.
...
- `Dario/Dario Pulgar Arriagada` and `Darío Pulgar Arriagada`: existing local context ties this form to separate medical, official-listing, photograph/title, passenger, and/or 2000 legal-notice clusters; this draft supplies no bridge.
- Passenger-list `Dario Pulgar` adult age 64 in 1953 creates a rough chronology hint for later review because an 1888 birth could be near age 64 in 1953, but the name and source context differ and the entry-172 row is not certified.
...
- Relationship conflict: Pulgar/Arriagada row would support one child-parent set; Burgos/de la Cruz row supports a completely different child-parent set.
- Chronology conflict: Pulgar/Arriagada row gives birth on 1888-03-08 and registration on 1888-04-07; Burgos/de la Cruz row gives birth on 1888-04-06 and registration on 1888-04-07.
