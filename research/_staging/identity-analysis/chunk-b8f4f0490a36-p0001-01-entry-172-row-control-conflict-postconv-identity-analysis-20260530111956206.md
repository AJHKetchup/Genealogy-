...
worker: "postconv-identity-analysis-20260530111956206"
...
- Row-control blocker: the assigned chunk and current converted Markdown attach entry `172` to different children, parents, birth events, places, and informants.
- Source-image blocker: the source packet says the full original PNG was not present in this checkout and available crop assets cannot certify the physical row number.
- Conversion blocker: the conflict is not a spelling variant. It is a derivative row/page-control conflict between a Pulgar/Arriagada row and a Burgos/de la Cruz row.
- Relationship blocker: no parent-child relationship from this staged draft is ready for canonical promotion until row control is certified.
- Father-field blocker: the assigned chunk reads `Jose del Carmen Pulgar S.`, while the source packet requires holding the field as `Jose del Carmen Pulgar` or `Jose del Carmen Pulgar [?]` until the suffix is image-certified.
- Pulgar-line blocker: this staged draft does not literally name `Dario Arturo Pulgar-Smith`, `Dario Arturo Pulgar`, `Dario Jose Pulgar-Arriagada`, `Dario Pulgar Arriagada`, or `Dario Pulgar A.`. Those names remain anti-conflation comparisons only.
- Jose/Juana blocker: existing wiki context contains `Jose del Carmen Pulgar`, `Juana Arriagada de Pulgar`, and other Jose/Juana candidates from separate records. This draft cannot merge or reconcile them by family context alone.
...
- Father: chunk transcription `Jose del Carmen Pulgar S.`; current proof boundary is `Jose del Carmen Pulgar` or uncertain suffix until image QA certifies the visible mark.
...
Interpretation: every identity-controlling field differs. This is a row-control and conversion-confidence conflict, not a same-person, duplicate-person, or name-normalization conflict.

## Hypotheses And Evidence

| Rank | Hypothesis | Evidence supporting | Conflicts and limits | Claim probability | Identity confidence | Canonical readiness |
| --- | --- | --- | --- | ---: | ---: | --- |
| 1 | Entry `172` is the Pulgar/Arriagada row. | The assigned chunk and source packet consistently state `Jose del Carmen Segundo Pulgar Arriagada`, father `Jose del Carmen Pulgar`, and mother `Juana Arriagada de Pulgar`. Existing wiki pages carry low-confidence/probable evidence from this cluster. | Current converted Markdown assigns the same entry number to a different family; full-image QA is unavailable here; father suffix is unresolved. | 0.68 | 0.62 | hold_for_conversion_qa |
| 2 | Entry `172` is the Burgos/de la Cruz row. | The current converted Markdown explicitly labels entry `172` as `Jose Miguel`, child of `Oswaldo Burgos` and `Concepcion de la Cruz`. | Directly conflicts with the assigned chunk and source packet; no Pulgar-family relevance. | 0.20 | 0.22 | hold_for_conversion_qa |
