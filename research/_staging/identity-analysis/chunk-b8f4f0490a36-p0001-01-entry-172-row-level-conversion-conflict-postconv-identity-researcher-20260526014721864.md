...
worker: "postconv-identity-analysis-20260526014721864"
...
1. The exact staged draft analyzed here is `research/_staging/conflicts/chunk-b8f4f0490a36-p0001-01-entry-172-row-level-conversion-conflict-postconv-evidence-extraction-20260526001440948.md`.
2. Row control is unresolved. The assigned chunk and source packet present entry 172 as a Pulgar/Arriagada birth row, while the current converted Markdown presents entry 172 as a Burgos/de la Cruz birth row.
3. This is a material identity and relationship conflict, not a spelling issue. The disagreement changes child identity, parents, birth date/time/place, parent residence, and informant.
4. The father field is not promotion-ready. The Pulgar-side father reading must be certified only to the visible extent: `Jose del Carmen Pulgar`, `Jose del Carmen Pulgar S.`, or `Jose del Carmen Pulgar [?]`.
5. Existing wiki pages already preserve related Pulgar/Jose/Juana/Dario clusters, but this analysis does not authorize any merge, rename, relationship promotion, or canonical fact strengthening.
6. No external research was performed. Raw sources, converted Markdown, chunks, and canonical wiki pages were not edited.

## Literal Evidence

From the staged conflict draft and source packet, the Pulgar/Arriagada reading for entry 172 is:

- Child: `Jose del Carmen Segundo Pulgar Arriagada`.
- Sex: `Hombre`.
- Birth: 8 March 1888, about 3 p.m., `Calle de Valdivia`.
- Father: `Jose del Carmen Pulgar S.` in the assigned chunk, with exact suffix still requiring QA.
...
- The staged source packet states that image review supports a Pulgar/Arriagada row for register page 58, entry 172, while preserving the father-suffix uncertainty.
- The staged conflict draft identifies the current converted Markdown as the conflicting Burgos/de la Cruz side.
...
| conflict_severity | 0.95 | Competing rows identify different children, parents, places, dates, and informants. |
...
- The staged conflict draft already frames the converted Markdown row as the conflict side.
...
- Existing wiki context has both `Juana Arriagada de Pulgar` and `Juana de Dios Amagada de Pulgar`, making the `Arriagada`/`Amagada` distinction a required anti-conflation check.

Conflicts and limits:

- This staged draft does not mention `Tulio Cesar Luis Jose`, the separate cluster linked to `Jose del Carmen Pulgar` and `Juana de Dios Amagada de Pulgar`.
...
- `Dario/Dario Pulgar Arriagada` and `Darío Pulgar Arriagada`: existing local context ties these forms to separate medical, official-listing, photograph/title, passenger, and/or 2000 legal-notice clusters; this draft supplies no bridge.
