...
worker: postconv-identity-analysis-20260525173531487
...
- Controlling-row blocker: the staged draft and referenced source packet say the assigned chunk supports entry `172` as `Jose del Carmen Segundo Pulgar Arriagada`, while the assigned converted Markdown says entry `172` is `Jose Francisco`, child of `Oswaldo Gomez` and `Rosario de la Cruz de la Maza`.
- This is not a spelling or name-variant issue. The competing readings differ by child name, parents, birth date, birth place, informant, and locality context.
- Father-field blocker: the ending after `Jose del Carmen Pulgar` remains unresolved. Preserve all proof-review outcomes: `Jose del Carmen Pulgar`, `Jose del Carmen Pulgar S.`, and `Jose del Carmen Pulgar [?]`.
- Relationship blocker: no child-parent relationship from this entry should be promoted until targeted conversion QA identifies the controlling row and proof review reruns the identity and relationship claims.
- Dario-line blocker: this entry does not name Dario, Arturo, Pulgar-Smith, Dario Jose, or Dario/Dario Pulgar Arriagada. It cannot bridge Dario-line identities by surname, family context, or an Arriagada surname pattern alone.
...
- Father: chunk reads `Jose del Carmen Pulgar S.`; proof-review context requires QA to decide among `Jose del Carmen Pulgar`, `Jose del Carmen Pulgar S.`, and `Jose del Carmen Pulgar [?]`.
...
Interpretation: the Pulgar/Arriagada reading is the leading family-relevant staged hypothesis, but the derivative outputs disagree too materially for canonical promotion or identity merging.
...
- Existing wiki context has pages for `Jose del Carmen Segundo Pulgar Arriagada`, `Jose del Carmen Pulgar`, and `Juana Arriagada de Pulgar`, confirming local relevance but not resolving this conversion conflict.
...
| `Dario Jose Pulgar-Arriagada` | Other staged source-packet context uses this source-title/file-identity name for an ICRC group photograph. | No support from this entry; do not infer from Pulgar/Arriagada surname pattern. | 0.01 |
| `Dario/Dario Pulgar Arriagada` | Other staged context includes `Dario Pulgar Arriagada` / name-form variants in institutional or local records. | No initial, occupation, chronology, residence, or relationship bridge appears here. | 0.01 |
| Jose/Juana parent candidates | This draft directly involves `Jose del Carmen Pulgar` and `Juana Arriagada de Pulgar`. | Relevant only as parent-candidate anti-conflation after row QA. | Jose 0.46; Juana 0.64 |
