...
worker: postconv-identity-analysis-20260525173741676
...
- Controlling-row blocker: the exact staged draft says the assigned chunk and source-image review support entry `172` as `Jose del Carmen Segundo Pulgar Arriagada`, while the assigned converted Markdown says entry `172` is `Jose Francisco`, child of `Oswaldo Gomez` and `Rosario de la Cruz de la Maza`.
- This is a row-level or file-assignment conflict, not a spelling variant. The competing readings differ in child name, parents, birth date, birth place, informant, and locality context.
- Father-field blocker: the ending after `Jose del Carmen Pulgar` remains unresolved. Preserve all proof-review outcomes requested in the staged draft: `Jose del Carmen Pulgar`, `Jose del Carmen Pulgar S.`, and `Jose del Carmen Pulgar [?]`.
- Relationship blocker: no child-parent relationship from this entry should be promoted until targeted conversion QA identifies the controlling row and renewed proof review handles the identity and relationship claims.
- Dario-line blocker: this entry does not name Dario, Arturo, Pulgar-Smith, Dario Jose, Dario J., or Dario/Darío Pulgar Arriagada. It cannot bridge Dario-line identities by surname, family context, or an Arriagada surname pattern alone.
...
- Father: chunk/source-packet context reads `Jose del Carmen Pulgar S.`, but the staged draft requires QA to decide among `Jose del Carmen Pulgar`, `Jose del Carmen Pulgar S.`, and `Jose del Carmen Pulgar [?]`.
...
Interpretation: the Pulgar/Arriagada reading is the leading family-relevant staged hypothesis, but the derivative outputs disagree too materially for canonical promotion, identity merging, or parent-candidate attachment.
...
- The staged draft explicitly identifies the chunk-supported row as the Pulgar/Arriagada entry.
...
- Existing wiki context has pages for `Jose del Carmen Segundo Pulgar Arriagada`, `Jose del Carmen Pulgar`, and `Juana Arriagada de Pulgar`, confirming local relevance but not resolving the conversion conflict.
...
This staged draft does not name any Dario candidate. It can only trigger a future double-check if separately reviewed lineage evidence connects the Jose/Juana family context to a specific Dario-line person.
...
| `Dario Jose Pulgar-Arriagada` | Other staged source-packet context uses this source-title/file-identity name for ICRC/Geneva materials. | No support from this entry; do not infer from Pulgar/Arriagada surname pattern. | 0.01 |
