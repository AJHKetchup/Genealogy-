...
worker: postconv-identity-analysis-20260530093216211
...
2. The requested `$genealogy-proof-review` skill/contract was not available as a local skill file in this checkout; this note follows the staged proof-review pattern present in existing local notes and keeps source readings separate from interpretation.
3. This is a conversion row-control conflict, not a same-person, duplicate-person, or name-variant merge candidate.
4. The assigned chunk records entry `172` as `Jose del Carmen Segundo Pulgar Arriagada`, while the current converted Markdown records entry `172` as `Jose Miguel`, child of `Oswaldo Burgos` and `Concepcion de la Cruz`.
5. The referenced full source PNG is absent in this checkout, so this analysis cannot certify the full physical row.
6. Existing staged crop assets support Pulgar/Arriagada parent and informant fields at crop level, but they do not replace full-page row-control QA and do not settle the possible father-name suffix after `Jose del Carmen Pulgar`.
7. Do not attach the Burgos/de la Cruz details to `Jose del Carmen Segundo Pulgar Arriagada`.
8. Do not use this row to merge or bridge `Dario Arturo Pulgar-Smith`, `Dario Arturo Pulgar`, `Dario Jose Pulgar-Arriagada`, `Dario/Dario Pulgar Arriagada`, canonical `Dario Pulgar Arriagada`, or any Jose/Juana parent candidates.

No external research was performed. Raw sources, converted Markdown, chunks, source packets, conversion reviews, and canonical wiki pages were not edited.
...
Targeted conversion-review note reading: staged crop assets visibly support `Jose del Carmen Pulgar`, `Juana Arriagada de Pulgar`, and `Ernesto Herrera L.` at crop level, while the original full-page image remains unavailable in this checkout.
...
- Existing canonical pages already contain auto-generated Pulgar/Arriagada evidence, but this staged conflict directs a hold; do not use this analysis to promote or normalize those pages.
...
- Existing local context has a separate `Jose del Carmen Pulgar` parent candidate for `Tulio Cesar Luis Jose`; this record does not prove whether the Jose candidates are the same person across records.
- Existing local context has Juana-name candidates in other Pulgar-line records; this row cannot silently normalize `Juana Arriagada de Pulgar` into another Juana person.
...
- Existing wiki/staged context includes `Dario Arturo Pulgar-Smith`, staged `Dario Arturo Pulgar`, `Dario Jose Pulgar-Arriagada`, and canonical `Dario Pulgar Arriagada`.

Conflicts and limits:

- The assigned row names no Dario.
- The row states no spouse, child, descendant, grandchild, occupation continuity, property, passenger, CV, or other bridge connecting it to a Dario identity.
...
| `Dario/Dario Pulgar Arriagada` / canonical `Dario Pulgar Arriagada` | Existing canonical stub records `Dario Pulgar Arriagada` in a 2000 legal/expropriation context. This 1888 birth-row candidate does not state Dario or later property context. | 0.03 | Do not attach; chronology and identity bridge required. |
...
| Other Jose/Juana parent candidates | Local Pulgar-line context has other Jose/Juana parent candidates in separate records. This staged row does not prove those are the same people. | 0.12-0.20 same-person range | Do not merge; require name, relationship, date, place, and source-continuity proof across records. |
...
- Name-variant evidence: `Jose del Carmen Pulgar S.` may contain a suffix or mark, but this analysis preserves only the broad certified name `Jose del Carmen Pulgar` pending full-image QA. `Juana Arriagada de Pulgar` is not silently normalized to another Juana candidate.
...
| 5 | Bridge to `Dario Jose Pulgar-Arriagada` or `Dario Pulgar Arriagada` | 0.03 | Unsupported from this row; no Dario attachment. |
