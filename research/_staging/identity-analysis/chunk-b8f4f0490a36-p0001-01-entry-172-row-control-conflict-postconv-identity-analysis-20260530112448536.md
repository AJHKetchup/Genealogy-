...
worker: "postconv-identity-analysis-20260530112448536"
...
- Row-control blocker: the assigned chunk and the current converted Markdown assign entry `172` to different children, parents, birth events, places, and informants.
- Source-image blocker: the revised source packet says the full original PNG is absent from this checkout and available crop assets cannot certify the physical row number.
- Conversion blocker: this is a derivative row/page-control conflict, not a same-person, duplicate-person, or spelling conflict.
- Relationship blocker: no parent-child relationship from this staged draft is ready for canonical promotion until physical row control is certified.
- Father-field blocker: the assigned chunk reads `Jose del Carmen Pulgar S.`, but the source packet instructs holding the field as `Jose del Carmen Pulgar` or `Jose del Carmen Pulgar [?]` until the suffix is visible and certified.
- Pulgar-line blocker: this staged draft does not literally name `Dario Arturo Pulgar-Smith`, `Dario Arturo Pulgar`, `Dario Jose Pulgar-Arriagada`, `Dario Pulgar Arriagada`, or `Dario Pulgar A.`. Those identities remain anti-conflation comparisons only.
- Jose/Juana blocker: existing wiki context contains `Jose del Carmen Pulgar`, `Juana Arriagada de Pulgar`, and other Jose/Juana candidates from separate evidence. This draft cannot merge or reconcile those candidates by family context alone.
...
- Father: chunk transcription `Jose del Carmen Pulgar S.`; proof boundary remains `Jose del Carmen Pulgar` with unresolved suffix until image QA.
...
Interpretation: every identity-controlling field differs. The two readings cannot both describe the same birth registration row.

## Hypotheses And Evidence

| Rank | Hypothesis | Evidence supporting | Conflicts and limits | Claim probability | Identity confidence | Canonical readiness |
| --- | --- | --- | --- | ---: | ---: | --- |
| 1 | Entry `172` is the Pulgar/Arriagada row. | The assigned chunk and revised source packet consistently state `Jose del Carmen Segundo Pulgar Arriagada`, father `Jose del Carmen Pulgar`, and mother `Juana Arriagada de Pulgar`. Existing wiki pages carry low-confidence/probable evidence from this cluster. | The current converted Markdown assigns the same entry number to a different family; full-image QA is unavailable here; father suffix is unresolved. | 0.68 | 0.62 | hold_for_conversion_qa |
| 2 | Entry `172` is the Burgos/de la Cruz row. | The current converted Markdown explicitly labels entry `172` as `Jose Miguel`, child of `Oswaldo Burgos` and `Concepcion de la Cruz`. | Directly conflicts with the assigned chunk and source packet; no Pulgar-family relevance is present. | 0.20 | 0.22 | hold_for_conversion_qa |
| 3 | The evidence represents a derivative row/page mismatch. | Child, parents, birth date/time/place, and informant all differ between derivative layers. | The mismatch source cannot be resolved without full-page image review. | 0.84 | 0.78 | hold_for_conversion_qa |
| 4 | The assigned-row father is the existing `Jose del Carmen Pulgar` candidate. | Same literal name base appears in the assigned row and an existing canonical stub exists. | Same-name evidence is insufficient; row control and father suffix remain unresolved; separate Jose candidates must stay separate. | 0.42 | 0.34 | not_ready |
...
| 6 | The assigned-row child is a Dario Pulgar identity candidate. | Only broad surname context: `Pulgar Arriagada`. | Child is named `Jose del Carmen Segundo Pulgar Arriagada`, not Dario; no Arturo, Smith, Dario Jose, profession, spouse, age, descendant, or chronology bridge appears in this draft. | 0.03 | 0.02 | not_ready |
...
| --- | --- | --- | --- |
| `Dario Arturo Pulgar-Smith` | No literal mention of Dario, Arturo, Smith, Alexander John Heinz, or the maternal-grandfather relationship. The canonical page is family-supplied and warns against automatic merges with similarly named Pulgar clusters. | Very low same-person support; probability 0.01. | Separate identity-bridge proof review using accepted identity-bearing records before attaching any staged Pulgar evidence to the family-supplied Pulgar-Smith page. |
| `Dario Arturo Pulgar` | No literal mention. Other staged CV contexts may mention document-level `Dario Arturo Pulgar`, but this entry does not bridge to that person. | Very low same-person support; probability 0.02. | Proof-review the CV identity bridge separately, then compare only if later accepted evidence supplies parentage, birth, age, or family links. |
...
- Name-variant issue: limited to the possible father suffix after `Jose del Carmen Pulgar`; this is not a Dario name-variant issue.
...
| Evidence quality | 0.60 | A civil birth registration would be strong evidence, but the available derivative layers conflict and the full source image is absent. |
...
Exact next step: a conversion-QA worker must restore or locate the full source image, compare the physical row for entry `172` against the assigned chunk and current converted Markdown, and certify whether the controlling row is Pulgar/Arriagada or Burgos/de la Cruz. The same QA pass must bound the father field as `Jose del Carmen Pulgar`, `Jose del Carmen Pulgar S.`, or `Jose del Carmen Pulgar [?]`.
