...
worker: postconv-identity-analysis-20260530210637213
...
- Page-control blocker: the staged conflict draft reports a derivative transcription disagreement for the same page reference. The referenced chunk and source packet support 1979-1970 work-history entries, while the conversion job page Markdown for page 5 supports 1999/1998 entries.
- Derivative-output blocker: the aggregate converted file contains both a page-5 1999/1998 section and a later 1979-1970 section in the same converted-file context. The chunk manifest also lists duplicate page-5 chunk records with different char counts and hashes.
- Promotion blocker: no occupational claim from either derivative page-5 text is promotion-ready until conversion QA establishes the controlling transcription against the authoritative page image or source PDF page.
...
- Relationship blocker: the staged draft supplies no birth, death, parent, spouse, child, sibling, grandchild, household, or other kinship evidence. It cannot resolve Jose/Juana parent candidates or any Pulgar-line relationship conflict.
- Canonical blocker: this staged draft does not prove that document-level `Dario Arturo Pulgar` is the same person as canonical `Dario Arturo Pulgar-Smith` or as any Pulgar-Arriagada candidate.
- Scope blocker: this analysis does not edit raw sources, converted Markdown, chunks, source packets, claims, or canonical wiki pages.
...
- Job and chunk manifests for the same conversion job.
- Canonical pages reviewed for local context: `wiki/people/dario-arturo-pulgar-smith.md`, `wiki/people/dar-o-pulgar-arriagada.md`, `wiki/people/jose-del-carmen-segundo-pulgar-arriagada.md`, `wiki/people/jose-del-carmen-pulgar.md`, `wiki/people/juana-arriagada-de-pulgar.md`, and `wiki/people/juana-de-dios-amagada-de-pulgar.md`.

Interpretive boundary: no external research was performed. Literal readings below remain separate from identity interpretation.

## Hypothesis 1: Page 5 Controls To The 1999/1998 Page-Markdown Text

Interpretation: the conversion job page Markdown may be the controlling page-5 transcription, and the 1979-1970 chunk/source-packet text may be stale, duplicated, or assigned to the wrong page.

Supporting evidence:

- `page-markdown/page-0005.md` labels itself as page 5 and transcribes `1999`, `1998 - 1999`, and `1997-1998` work-history entries.
- The conversion job manifest maps page 5 to `page-images/page-0005.jpg` and `page-markdown/page-0005.md`.
- The staged conflict correctly identifies that the page-markdown text cannot be silently reconciled with the 1979-1970 chunk text as the same page transcription.

Conflicts:

- The assigned chunk `CHUNK-fb0a0000636f-P0005-01` and staged source packet quote the 1979-1970 work-history sequence instead.
...
| conflict_severity | 0.88 | Wrong page control would misplace multiple work-history claims. |
| evidence_quality | 0.64 | Page Markdown and job manifest align, but assigned chunk/source packet disagree. |
| conversion_confidence | 0.38 | Text is readable, but control is unresolved. |
| claim_probability | 0.52 | Plausible controlling text, not proof-reviewed for promotion. |
...
Interpretation: the referenced chunk and source packet may be the intended page-5 evidence, while the page-markdown page-5 file may be a conflicting derivative.
...
- The aggregate converted file contains a later 1979-1970 section in the broader output.
...
- If the page-markdown and job manifest control page 5, this 1979-1970 text may belong to another page or to a stale derivative state.

Scores:

| score | value | rationale |
|---|---:|---|
| identity_confidence | 0.57 | Document-level source title supports the subject, but the page body is unnamed. |
...
- The source packet states that source identity is `Dario Arturo Pulgar`, identified by source title, while warning that page 5 itself does not repeat his name.
...
## Hypothesis 4: Dario Arturo Pulgar Is A Name Variant Of Dario Arturo Pulgar-Smith
...
- The derivative page-control conflict must be resolved before occupational details can even be weighed.
...
- Canonical `Dario Pulgar Arriagada` is a separate stub with a promoted evidence snapshot tied to a 2000 expropriation resolution, not to this CV page.
...
- Relationship conflicts cannot be resolved from this page because the page has no relationship evidence.
...
- Same-person evidence: insufficient for a canonical attachment to `Dario Arturo Pulgar-Smith`.
