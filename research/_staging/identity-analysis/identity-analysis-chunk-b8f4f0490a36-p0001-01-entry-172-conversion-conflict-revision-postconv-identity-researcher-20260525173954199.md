...
worker: postconv-identity-analysis-20260525173954199
...
- Controlling-row blocker: the exact staged draft says the assigned chunk supports entry `172` as `Jose del Carmen Segundo Pulgar Arriagada`, while the assigned converted Markdown records entry `172` as `Jose Francisco`, child of `Oswaldo Gomez` and `Rosario de la Cruz de la Maza`.
- This is not a name variant. The two derivative readings disagree on child name, parents, birth date, birth place, informant, locality, and family relevance.
...
- Relationship blocker: do not promote child-parent relationships for this entry until conversion QA identifies the controlling row and proof review reruns relationship claims.
- Dario-line blocker: this entry does not name Dario, Arturo, Pulgar-Smith, Dario Jose, Dario J., or Dario/Darío Pulgar Arriagada. It cannot bridge Dario-line identities by surname or family-context hint alone.
...
- Father: chunk/source-packet context reads `Jose del Carmen Pulgar S.`, but the staged draft requires QA to decide the exact ending after `Pulgar`.
...
Interpretation: the Pulgar/Arriagada reading is the leading family-relevant staged hypothesis because the chunk and current source packet agree, but the converted-file conflict is too material for canonical promotion or identity merging.
...
- Prior local conversion review notes report that image reread supported the Pulgar/Arriagada row, while still requiring father-field QA.
...
| evidence_quality | 0.70 | Detailed local staged evidence plus prior review notes, but derivative outputs still disagree. |
...
| `Dario Jose Pulgar-Arriagada` | Other staged source-title/file context uses this name for ICRC/Geneva materials. | No support from this entry; do not infer from Pulgar/Arriagada surname pattern. | 0.01 |
| `Dario/Darío Pulgar Arriagada` | Existing wiki context includes `Darío Pulgar Arriagada` in a 2000 Chiguayante expropriation notice; other staged notes discuss Dario/Darío name-form variants. | No initial, occupation, chronology, residence, property, or relationship bridge appears here. | 0.01 |
