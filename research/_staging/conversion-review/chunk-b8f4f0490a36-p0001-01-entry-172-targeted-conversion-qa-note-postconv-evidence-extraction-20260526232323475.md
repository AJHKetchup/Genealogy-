...
The controlling physical row for entry `172` is the middle row on the left page, register page 58. The row is the Pulgar/Arriagada birth registration, not the Burgos/de la Cruz row recorded in the current converted Markdown.

The current converted Markdown is stale, row-shifted, or sourced from a different image/page set for this source identity. It records entry `172` as `Jose Miguel`, child of `Oswaldo Burgos` and `Concepcion de la Cruz`; the source image and assigned chunk instead show entry `172` as `Jose del Carmen Segundo Pulgar Arriagada`, child of `Jose del Carmen Pulgar` and `Juana Arriagada de Pulgar`.
