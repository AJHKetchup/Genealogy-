...
Fresh source-image certification could not be completed in this pass. The image path named by the task and chunk is not present in `raw/sources`; the only PNG currently visible in that directory is a different 1889 certificate image.
