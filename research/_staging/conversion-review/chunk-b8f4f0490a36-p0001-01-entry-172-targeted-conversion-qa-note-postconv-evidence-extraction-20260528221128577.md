...
worker: "postconv-evidence-extraction-20260528221040889"
...
conversion_qa_concern: "Current converted Markdown assigns entry 172 to a Burgos/de la Cruz row; the original image and assigned chunk support the Pulgar/Arriagada physical row."
...
uncertainty: "Father field is certified only through Jose del Carmen Pulgar. A possible trailing suffix or mark after Pulgar is not promoted."
...
The current converted Markdown instead records entry `172` as `Jose Miguel`, father `Oswaldo Burgos`, mother `Concepcion de la Cruz`, and birth on 6 April 1888. That derivative reading does not control this source packet or the staged family claims for the Pulgar/Arriagada row.

## Father-Field Boundary

The image supports the father name through `Jose del Carmen Pulgar`. The assigned chunk's `Jose del Carmen Pulgar S.` is preserved as a derivative reading, but the terminal `S.` is not certified here. Staged claims and relationships below use only `Jose del Carmen Pulgar`.
