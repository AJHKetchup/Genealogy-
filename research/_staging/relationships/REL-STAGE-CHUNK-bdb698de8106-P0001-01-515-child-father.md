...
child: Rosa Elvira del Carmen
parents: [Pedro Pablo Leiva]
...
# Relationship Candidate: Entry 515 Child And Father
...
Low for the converted surname and child name. Targeted image review appears closer to `Pedro Pablo Neira` and a child-name reading beginning `Neira Elvira / Laura de la Cruz...`; the row is also bottom-cropped.

## Uncertainty

Hold until conversion QA confirms whether the converted `Rosa Elvira del Carmen` / `Pedro Pablo Leiva` readings are valid or should be corrected to the image-visible `Neira` readings.
