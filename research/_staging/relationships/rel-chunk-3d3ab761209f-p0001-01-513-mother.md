...
Literal support: child field begins with source-visible `Pulgar Ama...`; mother field most likely reads `Juana de Dios Amagada de Pulgar`.

Conversion confidence / QA concern: assigned chunk says `Amagada`; converted Markdown says `Amador`; image review favors `Amagada` but does not remove the need for targeted conversion QA.

Uncertainty: do not promote the relationship until the child's registered name and the mother's surname are reconciled from the image.
