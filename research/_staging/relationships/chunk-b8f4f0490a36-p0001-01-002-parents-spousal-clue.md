...
- Literal support: assigned chunk gives father as `Jose del Carmen Pulgar S.`; this source-image reread supports `Jose del Carmen Pulgar` without a clearly visible suffix. Mother is recorded as `Juana Arriagada de Pulgar`. The converted Markdown file instead gives entry 172 as `Oswaldo Gomez` and `Emilia de la Cruz`.
