---
type: relationship_candidate
status: draft
relationship_type: spouse
person_a: "Dario Pulgar (adult passenger, age 64)"
person_b: "Dorothy Pulgar"
source_packet: "research/_staging/source-packets/sp-chunk-a4cd3c3e5c16-p0001-01-andes-pulgar-passenger-list.md"
confidence: low
promotion_recommendation: do_not_promote
---

# Relationship Candidate: Dario Pulgar and Dorothy Pulgar

Source path: `raw/sources/Passenger List, Royal Mail Lines Limited, August 7, 1953.png`

Converted file: `raw/converted/ca5a5078ab-passenger-list-royal-mai-passenger-list-royal-mail-lines-limited-august-7-1953.codex.md`

Chunk/page reference: `raw/chunks/ca5a5078ab-passenger-list-royal-mai-passenger-list-royal-mail-lines-limited-august-7-1953-codex/page-0001-chunk-01.md`, `CHUNK-a4cd3c3e5c16-P0001-01`, image page 1, passenger list page `P.M. 25.`

## Candidate

Dario Pulgar and Dorothy Pulgar may be spouses, but this passenger list should not be used alone to promote a spouse relationship.

## Literal Support

```text
| | " | PULGAR Dario | " | 64 `92` | ... | Bedford Corner Hotel, London. | Medical | + | ... | Chile | Chile |
| | " | " Dorothy | " | | 49 `92` | ... | " | - | + | ... | " | " |
```

The age columns used for both adults are labeled for adults `Accompanied by husband or wife`; Dorothy's surname is represented by ditto mark under the preceding Pulgar surname.

## Conversion Confidence And QA Concern

Low. The page is queued for reread and the original image was not available at the recorded path during extraction.

## Uncertainty

The record does not explicitly identify Dario and Dorothy as each other's spouses. The table structure supports that each adult was accompanied by a husband or wife, but does not name the spouse.

## Promotion Recommendation

Do not promote this relationship from this source alone.
