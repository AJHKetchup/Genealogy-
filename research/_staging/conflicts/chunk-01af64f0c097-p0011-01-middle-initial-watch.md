---
type: conflict_candidate
status: draft
conflict_type: identity_disambiguation_watch
subject: "Dario J. Pulgar Arriagada"
source_packet: "research/_staging/source-packets/chunk-01af64f0c097-p0011-01-dario-j-pulgar-arriagada-medico-cirujano.md"
source: "raw/sources/Anales de la Universidad de Chile, Session of the Council of Public Instruction, September 1918..pdf"
converted_file: "raw/converted/ca753b8b14-anales-de-la-universidad-p0004-0023-anales-de-la-universidad-de-chile-session-of-the-council-of-public-instruction-september-1918-pages-4-23.codex.md"
chunk: "raw/chunks/ca753b8b14-anales-de-la-universidad-p0004-0023-anales-de-la-universidad-de-chile-sess-af33a57786/page-0011-chunk-01.md"
chunk_id: CHUNK-01af64f0c097-P0011-01
page_reference: "source page 11"
confidence: medium
promotion_recommendation: do_not_promote
---

# Conflict Candidate: Middle Initial And Identity Watch

- Literal support: the converted list entry reads `» Dario J. Pulgar Arriagada`.
- Conversion confidence/QA concern: medium; the page needs reread and the referenced page image is missing.
- Uncertainty: The middle initial `J.` and maternal surname `Arriagada` may be important disambiguators. This source alone does not prove identity with any other Dario Pulgar or Pulgar Arriagada person in the research set.
- Promotion recommendation: do not promote as a canonical conflict; use as an identity-review caution after conversion QA.
