---
type: conflict_candidate
status: draft
conflict_type: no_direct_genealogical_conflict
confidence: high
subject: League of Nations registry file cover R 3578 / 50 5569 5569 Jacket No. 5
source: raw/converted/ca09a98281-r3578-50-5569-5569-jacke-p0001-0025-r3578-50-5569-5569-jacket5-pages-1-25.codex.md
source_path: raw/sources/R3578-50-5569-5569-Jacket5.pdf
source_packet: research/_staging/source-packets/sp-stage-chunk-3f469b56e502-p0002-01-league-of-nations-routing-slip.md
chunk: raw/chunks/ca09a98281-r3578-50-5569-5569-jacke-p0001-0025-r3578-50-5569-5569-jacket5-pages-1-25-codex/page-0002-chunk-01.md
chunk_id: CHUNK-3f469b56e502-P0002-01
page_reference: "page 2"
conversion_confidence: moderate
promotion_recommendation: do_not_promote
tags: [conflict-candidate, staging, negative-evidence, source-context]
---

# Conflict Candidate: No Direct Genealogical Conflict

## Candidate Result

No direct genealogical conflict is stated in this assigned chunk. The page contains source-cover metadata and a circulation log, not competing facts about a person, event, or family relationship.

## Literal Support Reviewed

```text
GENERAL AND MISCELLANEOUS
```

```text
Sujet:
Conférence Diplomatique (Juillet 1929) appelée à revoir la
Convention de Genève de 1906 pour l'amélioration du sort des
blessés et malades dans les armées en campagne et d'élaborer
un code des prisonniers de guerre.
```

```text
| Transmis à<br>Referred to | Date | Transmis à<br>Referred to | Date | Transmis à<br>Referred to | Date |
```

## Source Path

- Source path: `raw/sources/R3578-50-5569-5569-Jacket5.pdf`
- Converted file: `raw/converted/ca09a98281-r3578-50-5569-5569-jacke-p0001-0025-r3578-50-5569-5569-jacket5-pages-1-25.codex.md`
- Chunk/page reference: `CHUNK-3f469b56e502-P0002-01`, page 2.

## Conversion Confidence And QA Concern

Reading confidence is moderate to high. The conversion flags uncertain routing-log readings, but those uncertainties do not create a genealogical conflict.

## Uncertainty

There may be source-metadata uncertainty around detailed routing entries and struck-through dates. That should be handled as conversion QA, not a genealogical conflict.

## Promotion Recommendation

Do not promote.
