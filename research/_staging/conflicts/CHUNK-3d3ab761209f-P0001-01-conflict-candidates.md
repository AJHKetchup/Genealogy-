...
- Entry 513 child: assigned chunk has `Pulgar Amagada / Jose Luis`; converted file has `Isidoro del Carmen Diaz`. The image supports a `Pulgar Ama...` surname line and a given-name line beginning `Jose ...`, but does not securely support `Jose Luis`; the name should remain held.
...
- Entry 515 child and parents: assigned chunk has `Neira Ulloa / Laura de la Cruz`, father `Pedro Pablo Neira`, mother `Carmen Ulloa`; converted file has `Rosa Elvira del Carmen`, father `Pedro Pablo Leiva`, mother `Carmen Leiva`. The image supports the `Neira=emendado= / vale=` note and the father area, but the lower mother area remains cropped/insufficiently visible for promotion.
