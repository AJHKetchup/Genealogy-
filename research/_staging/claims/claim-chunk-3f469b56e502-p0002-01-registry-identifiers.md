---
type: claim
status: draft
claim_type: source_identifier
confidence: 8.5
subject: League of Nations registry file cover R 3578 / 50 5569 5569 Jacket No. 5
predicate: bears_classification_and_numbers
object: "GENERAL AND MISCELLANEOUS; 50 | 5569 | 5569"
date: null
place: null
source: raw/converted/ca09a98281-r3578-50-5569-5569-jacke-p0001-0025-r3578-50-5569-5569-jacket5-pages-1-25.codex.md
source_path: raw/sources/R3578-50-5569-5569-Jacket5.pdf
source_packet: research/_staging/source-packets/sp-stage-chunk-3f469b56e502-p0002-01-league-of-nations-routing-slip.md
chunk: raw/chunks/ca09a98281-r3578-50-5569-5569-jacke-p0001-0025-r3578-50-5569-5569-jacket5-pages-1-25-codex/page-0002-chunk-01.md
chunk_id: CHUNK-3f469b56e502-P0002-01
page_reference: "page 2"
conversion_confidence: high
promotion_recommendation: promote_after_review
tags: [claim, staging, source-identifier, archival-file-cover]
---

# Atomic Claim: Registry Identifiers

## Claim

The file cover is classified as `GENERAL AND MISCELLANEOUS` and bears stamped number boxes reading `50 | 5569 | 5569`.

## Literal Source Support

```text
[Stamped text at top left]
GENERAL AND MISCELLANEOUS

[Stamped numbers in boxes at top right]
50 | 5569 | 5569
```

## Source Path

- Source path: `raw/sources/R3578-50-5569-5569-Jacket5.pdf`
- Converted file: `raw/converted/ca09a98281-r3578-50-5569-5569-jacke-p0001-0025-r3578-50-5569-5569-jacket5-pages-1-25.codex.md`
- Chunk/page reference: `CHUNK-3f469b56e502-P0002-01`, page 2.

## Conversion Confidence And QA Concern

Reading confidence is high for the stamped classification and number boxes. Proof review should confirm the stamped numbers against the page image before canonical source-description promotion.

## Uncertainty

This is an administrative source identifier claim, not a person or family relationship claim.

## Promotion Recommendation

Promote after proof review as source-description metadata.
