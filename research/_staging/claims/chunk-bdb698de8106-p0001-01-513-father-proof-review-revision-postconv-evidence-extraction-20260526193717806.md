---
type: claim
status: draft
claim_type: parent
subject: entry 513 child identity unresolved
predicate: father
object: Jose del Carmen Pulgar
source: raw/sources/Registro de Nacimientos, Circunscripción de Los Ángeles, Chile, 1889, Certificate No. 513..png
source_packet: research/_staging/source-packets/chunk-bdb698de8106-p0001-01-entry-513-515-proof-review-revision-postconv-evidence-extraction-20260526193717806.md
converted_file: raw/converted/ca05d0627a-registro-de-nacimientos-registro-de-nacimientos-circunscripci-n-de-los-ngeles-chile-1889-certificate-no-513.codex.md
chunk: raw/chunks/ca05d0627a-registro-de-nacimientos-registro-de-nacimientos-circunscripci-n-de-los-nge-5ed7132d63/page-0001-chunk-01.md
chunk_id: CHUNK-bdb698de8106-P0001-01
page_reference: page 1; register page 172; entry 513
confidence: 0.78
promotion_recommendation: hold_for_conversion_qa
---

# Atomic Claim Revision: Entry 513 Father

## Literal Support

```text
Nombre del padre.
Jose del Carmen Pulgar
Nac. Chileno Prof. Agricultor
Domicilio. Calle Colon
```

## Conversion Confidence / QA Concern

Medium for the father-name field, low for canonical parentage. The father/declarant fields carry the family-relevant `Pulgar` evidence, but child identity and other row fields remain in conversion conflict.

## Uncertainty

Do not attach this father to a canonical child profile until targeted conversion QA resolves the child-name field.

## Promotion Recommendation

`hold_for_conversion_qa`.
