...
Entry 513 is relevant to the Pulgar family because the row records a child in a Pulgar household and identifies Jose del Carmen Pulgar / Jose del C. Pulgar as father and declarant. The same row names Juana de Dios Arriagada de Pulgar as mother in the image-reviewed reading. The child appears to be a Pulgar Arriagada child, with the given-name line plausibly reading Jose Dario, but the full child name is not yet QA-certified.
...
Nombre: Pulgar Arriagada / Jose Dario[?]
Sexo: Masculino
...
Conversion confidence remains low. The chunk, converted file, prior drafts, and image-reviewed reading disagree on the child full name, mother surname, and birth date/time. The image supports the Pulgar family row and appears to favor `Arriagada` for the child and mother surname, but this evidence extraction task is not a conversion update and does not edit the converted Markdown or chunk.

## Uncertainty

The child's given-name line appears compatible with `Jose Dario`, but it should be confirmed with targeted row-level conversion QA or a high-resolution crop. Entry 515 is a separate lower row and should not be used to extract Pulgar-family claims from this packet.

## Promotion Recommendation

`hold_for_conversion_qa`. Use this packet as a correction map and blocker summary. Do not promote entry 513 claims, parent-child relationships, person-page edits, family-page edits, or identity-cluster links until a QA-certified transcription and proof review exist.
