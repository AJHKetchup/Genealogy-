...
conversion_confidence: "medium_high_after_image_row_control"
conversion_qa_concern: "Source image and assigned chunk support the Pulgar/Arriagada row for entry 172; current converted Markdown is materially stale or row-shifted and gives a Burgos/de la Cruz row. Father field is certified only as Jose del Carmen Pulgar; do not promote the trailing S. from this packet."
...
Direct review of the original page image identifies entry `172` as the second row on register page 58. That row is the Pulgar/Arriagada row and supports the child, mother, birth date/time/place, and informant readings in the assigned chunk. The father field is visible as `Jose del Carmen Pulgar`; the final mark after `Pulgar` is not certified as `S.` in this packet.

## Converted Markdown Conflict

The current converted Markdown records entry `172` as `Jose Miguel`, child of `Oswaldo Burgos` and `Concepcion de la Cruz`, born 6 April 1888. That is a material derivative conflict and should not control this entry against the source image. The conflict remains relevant for audit, but the staged claims below use the image-controlled row.

## Promotion Recommendation

Promote after proof review if the reviewer accepts the image-row-control note. Do not promote the father suffix `S.` or attach this child to any Dario Pulgar identity from this packet alone.
