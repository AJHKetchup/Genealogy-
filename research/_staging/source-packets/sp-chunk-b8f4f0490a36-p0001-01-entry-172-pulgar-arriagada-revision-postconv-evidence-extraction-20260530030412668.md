...
conversion_qa_concern: "The current converted Markdown materially conflicts with the assigned chunk by recording entry 172 as Jose Miguel, child of Oswaldo Burgos and Concepcion de la Cruz. The original source image path did not resolve in this worker session, but existing staged crop assets show the Pulgar/Arriagada parent and informant fields for the controlling row. Preserve the father field only as Jose del Carmen Pulgar; do not certify the trailing suffix after Pulgar."
uncertainty: "High for promotion because proof review requires targeted QA plus proof review before canonical use. Medium for father-field suffix after Pulgar. Low for family relevance if the Pulgar/Arriagada row remains controlling."
promotion_recommendation: hold_for_conversion_qa
---

# Source Packet: Entry 172 Pulgar/Arriagada Birth Registration

This packet stages the family-relevant evidence for the Pulgar/Arriagada row while preserving the conflict between derivative transcripts.

## Row Summary

The assigned chunk identifies entry `172` on register page `58` as a birth registration for `Jose del Carmen Segundo Pulgar Arriagada`.

- Child: `Jose del Carmen Segundo Pulgar Arriagada`.
- Sex: `Hombre`.
- Registration date: `Siete de Abril de mil ochocientos ochenta i ocho`.
- Birth: `Ocho de Marzo de mil ochocientos ochenta i ocho`, at `tres de la tarde`, at `Calle de Valdivia`.
- Father: preserve only as `Jose del Carmen Pulgar`; the possible suffix after `Pulgar` remains uncertified.
...
Existing staged crop assets under `research/_staging/conversion-review-assets/` show the lower parent/informant fields for the Pulgar row, including `Jose del Carmen Pulgar`, `Juana Arriagada de Pulgar`, and `Ernesto Herrera L.`. The crop supports the base father name but does not make a trailing suffix after `Pulgar` promotion-ready.
...
Keep this packet and dependent claims/relationships at `hold_for_conversion_qa`. Rerun proof review before any canonical claim, parent-child relationship, identity merge, Dario-line attachment, or wiki update.
