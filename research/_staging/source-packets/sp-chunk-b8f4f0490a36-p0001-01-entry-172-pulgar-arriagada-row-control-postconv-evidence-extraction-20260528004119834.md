...
uncertainty: "Low for row control and main Pulgar/Arriagada birth facts; moderate for the possible terminal father suffix after Pulgar; proof review still required before canonical promotion."
...
Direct image review confirms physical row `172` on register page 58 is the Pulgar/Arriagada row. The image supports the child's name, sex, registration date, birth date/time/place, mother, parent residence, and compareciente. The father is staged only as `Jose del Carmen Pulgar`; the possible terminal suffix after `Pulgar` is not certified.
