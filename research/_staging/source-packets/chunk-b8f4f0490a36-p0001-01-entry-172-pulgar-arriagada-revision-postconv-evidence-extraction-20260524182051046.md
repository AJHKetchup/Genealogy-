...
conversion_qa_concern: "Material row-level conflict: the assigned converted Markdown transcribes entry 172 as Jose Francisco, father Oswaldo Gomez, mother Emilia de la Cruz, born 26 March 1888 at En esta. The assigned chunk and source image support the Pulgar/Arriagada row. Targeted conversion QA must reconcile or supersede the converted Markdown and explicitly record whether the father field reads Jose del Carmen Pulgar, Jose del Carmen Pulgar S., or Jose del Carmen Pulgar [?]."
uncertainty: "Low for the Pulgar/Arriagada row identity in the current chunk and source image; medium for the exact father-name suffix and parent-residence spelling; high for canonical readiness until conversion QA and proof review are complete."
...
Source-image review in this extraction pass supports the controlling row as the Pulgar/Arriagada row on register page 58, entry 172. The image supports `Jose del Carmen Pulgar` as the broad father reading, but this evidence-extraction pass does not settle whether a final `S.` or other mark after `Pulgar` is present.

## Derivative Transcript Conflict

The assigned converted Markdown records entry 172 as an unrelated `Jose Francisco` / `Oswaldo Gomez` / `Emilia de la Cruz` birth at `En esta`. That is a material row-level conflict with the assigned chunk and source-image evidence, not a spelling variant.
