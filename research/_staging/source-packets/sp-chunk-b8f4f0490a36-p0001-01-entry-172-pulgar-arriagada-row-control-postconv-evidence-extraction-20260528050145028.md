...
conversion_qa_concern: "Original image confirms physical row 172 is the Pulgar/Arriagada row; current converted Markdown gives a conflicting Burgos/de la Cruz row for entry 172."
uncertainty: "Low for row control and main birth facts; moderate for any terminal suffix after the father's surname Pulgar."
...
This birth registration directly supports the Pulgar/Arriagada family narrative by recording a male child's birth, stated parents, parent domicile, and a compareciente who was present at the birth.
...
Direct image review confirms that physical row `172` on register page 58 is the Pulgar/Arriagada row. The image supports the child, sex, registration date, birth date/time/place, mother, parent domicile, and compareciente. The father is staged only as `Jose del Carmen Pulgar`; the possible suffix after `Pulgar` is not promoted from this review.
...
`promote_after_review`: promote only after proof review accepts the row-control note and the limited father-field reading.
