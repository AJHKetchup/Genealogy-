...
conversion_confidence: medium-high for typed body text; medium for handwritten signature/name form
conversion_qa_concern: Controller priority includes qc:reread-page; verify Dario's exact name form, the handwritten signature, the title/header, and the year 1917 before canonical promotion.
uncertainty: The typed article reads "DR DARIO PULGAR A," while the visual note reports a handwritten signature transcribed as "DR. DARIO PULGARA"; this may represent Dario Pulgar A. or an abbreviated maternal surname, but identity normalization requires proof review.
...
The assigned chunk supports property, place, occupation, inheritance, and limited unnamed-parent relationship evidence for Dr. Dario Pulgar A. It does not name Dario's parents, spouse, children, household members, or a full expanded surname.

## QA Notes

The controller flagged `reread-page`. Hold all promotable conclusions until proof review confirms the typed name line, signature, year, and page context from the source image.
