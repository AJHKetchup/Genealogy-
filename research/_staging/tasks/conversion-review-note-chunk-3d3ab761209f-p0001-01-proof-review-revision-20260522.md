...
- Entry 513 birth field: image review appears closer to `Junio veinte i dos` than the assigned chunk's `Junio veinte i seis`; the exact time phrase remains faint and must be checked from a targeted crop.
...
- Entry 513 child: assigned chunk `Pulgar Amagada / Jose Luis`; converted Markdown `Tulio Cesar Luis Jose`.
- Entry 513 birth date/time: assigned chunk `Junio veinte i seis ... a las cuatro i media de la tarde`; converted Markdown `Junio veinte i dos ... a las cuatro i veinte de la mañana`; image review currently favors `Junio veinte i dos` but does not settle the time.
...
- Entry 514 child/place/witness: assigned chunk `Riquelme Aviles`, `Calle Sanegueso`, `Benjamin Utiera`; converted Markdown `Rigoberto Juan Bautista`, `Calle San Joaquin`, `Benjamin Utria`.
- Entry 514 father: image and assigned chunk support `Se ignora`; converted Markdown's `Belisario Riquelme` should not be used as a parent-child relationship from this evidence packet.
- Entry 515 mother: assigned chunk `Carmen Ulloa`; converted Markdown `Carmen Fuentes`; image crop does not visibly confirm the field.
